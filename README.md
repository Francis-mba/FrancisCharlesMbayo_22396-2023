# 🌐 Francis Charles Mbayo - Personal Portfolio Website

## 👤 Student Information
- **Name:** Francis Charles Mbayo  
- **Student ID:** 22396/2023
- **Section:** (Add your section here)

---

## 🚀 Live Demo
👉 [View My Portfolio Live](https://francischarlesmbayo.github.io/FrancisCharlesMbayo_22396/)

> If the page does not appear, go to **Settings → Pages** on GitHub,  
> set the **Source** to `main` branch, and choose the `/ (root)` folder.

---

## 💻 Code Link
👉 [View Source Code on GitHub](https://francischarlesmbayo.github.io/FrancisCharlesMbayo_22396/))

---

## 📸 Screenshot
![Portfolio Screenshot]("img/Screenshot 2025-11-12 004113.png")
![Portfolio Screenshot]("img/Screenshot 2025-11-12 004113.png")

---

## 🧠 Project Overview
This is my **personal portfolio website**, created to showcase my **skills, education, projects, and certificates**.  
It provides an overview of my background and professional achievements with a clean and responsive design.

---

## 🧩 Pages Included
- 🏠 **Home:** Personal introduction and overview  
- 🎓 **Education:** Academic qualifications  
- 💼 **Projects:** My completed and ongoing projects  
- 🧠 **Skills:** Technical and soft skills  
- 📜 **Certificates:** Earned certificates and recognitions  
- 📞 **Contact:** Ways to get in touch  

---

## ⚙️ Steps to Run the Project Locally
1. Clone this repository:
   ```bash
   git clone https://github.com/FrancisCharlesMbayo/FrancisCharlesMbayo_22396.git
